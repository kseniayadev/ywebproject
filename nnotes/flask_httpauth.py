    ...
    def login_required(self, f):
        ...
        def decorated(*args, **kwargs):
            ...
            if request.method != 'OPTIONS':  # pragma: no cover
                ...
                res = self.authenticate(auth, password)
                if not res[0]:
                    # Clear TCP receive buffer of any pending data
                    request.data
                    return self.auth_error_callback(res[1])
...
class HTTPBasicAuth(HTTPAuth):
    ...
    def authenticate(self, auth, stored_password):
        ...
        return client_password is not None and \
            client_password == stored_password, None
...
class HTTPDigestAuth(HTTPAuth):
    ...
    def authenticate(self, auth, stored_password_or_ha1):
        ...
        return response == auth.response, None
...
class HTTPTokenAuth(HTTPAuth):
    ...
    def authenticate(self, auth, stored_password):
        ...
        if self.verify_token_callback:
            return self.verify_token_callback(token), None
        return False, None
